...
################################################################################
## average per region
all.res <- pdsload("data/derivatives/voxel-wise-drug-spearman-voxel-labeled.rds.pxz")
ss <- all.res %>% ungroup() %>% select(-c(1:3, cerebrA_structure,roi_name)) %>%
  group_by(synthseg_structure) %>% summarise_all(.funs=median) %>%
  ungroup() %>% pivot_longer(cols = -1, names_to = "drug")
